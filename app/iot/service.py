...
class IOTService:
    ...
    async def simult_register_device(self, devices: list) -> list:
        tasks = []
        for device in devices:
            task = asyncio.create_task(self.register_device(device))
            tasks.append(task)
    ...
    async def run_program(self, programs: list) -> None:
